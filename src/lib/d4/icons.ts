...
export function slotIcon(slot: string) {
  return `/images/d4/slots/${slot}.svg`;
}

export function uniqueIcon(name: string) {
  return `/images/d4/uniques/${slugify(name)}.svg`;
}
...
export function itemIcon(item: { uniqueName?: string; slot: string; name?: string }) {
  ...
  if (item.name && /^(harlequin|heir of|shroud of false|tyrael|ring of starless|andariel|doombringer|the grandfather|melted heart|nesekem|ahavarion)/i.test(item.name)) {
